...
$email = app->create_email(
  to => '"Mark Tyrrell" <mark@tyrrminal.dev>',
);
like($email->as_string, qr/To: "Mark Tyrrell" <mark\@tyrrminal.dev>\r$/m, 'default resolution of name + address');

like(
    warning { app->create_email( to => 'mark') },
    qr/^Argument contains empty address/,
    "default resolution of non-address"
);
...
like($email->as_string, qr/Cc: mark\@tyrrminal.dev, admin\@tyrrminal.dev, t\@tyrrminal.dev\r$/m, 'recursive custom resolution of group');
